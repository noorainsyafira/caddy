                        ...
                        <table class="table table-bordered table-hover" id="dataTable">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    @if($employee->attendancesToday->count())
                                        <td><h6 class="text-center"><span class="badge badge-pill badge-success">Recorded</span></h6></td>
                                        <td>
                                            @foreach($employee->attendancesToday as $attendance)
                                                <div class="d-flex justify-content-between align-items-center mb-1">
                                                    <h6 class="text-center mr-3">
                                                        Recorded on {{ $attendance->shift }} shift for {{ $attendance->rounds }} rounds.
                                                    </h6>
                                                    <button 
                                                        class="btn btn-flat btn-danger"
                                                        data-toggle="modal"
                                                        data-target="#deleteModalCenter{{ $attendance->id }}"
                            ...
                            @foreach($employees as $i => $employee)
                                <!-- Modal -->
                                @foreach($employee->attendancesToday as $attendance)
                                <div class="modal fade" id="deleteModalCenter{{ $attendance->id }}" tabindex="-1" role="dialog" aria-labelledby="deleteModalCenterTitle1{{ $attendance->id }}" aria-hidden="true">
                                        ...
                                        <div class="modal-content">
                                            <div class="card card-danger">
                                                ...
                                                <div class="card-body text-center d-flex" style="justify-content: center">
                                                    ...
                                                    <form 
                                                    action="{{ route('admin.employees.attendance.delete', $attendance->id) }}"
                                                    method="POST"
                                                    >
                                                    @csrf
                                                    @method('DELETE')
